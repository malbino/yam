...
if (isset($_GET['id_persona']))
{
  ...
  join rol r on p.rol = r.id_rol join privilegios p2 on r.id_rol = p2.rol_id_rol join funcionalidad f
    on p2.funcionalidad_id_funcionalidad = f.id_funcionalidad where u.usuario='$this->usuario'";
        ...
        {
            $sql = "select * from usuario WHERE usuario ='$u' AND clave='$c' and activousuario=1;";
  ...
  where u.usuario='$u'";
        ...
        public function actualizar ($p){
            $sql="UPDATE persona set nombre='$p[nombre]',papellido='$p[appaterno]',sapellido='$p[apmaterno]',direccion='$p[direccion]'
            ,telefono=$p[telefono] where id_persona=$p[id_per]";
            $this->obj_con->con_retorno($sql);
            $_SESSION['error']="modificado";
            echo "<script> window.location.href='../admin/docs/imprimir_cuenta.php?c=$p[nick]';</script>";
        }
    }
}